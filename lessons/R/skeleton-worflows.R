...
rename_holothuria <- function(input_path="data/split_files",
    ...
    oldNm <- list.files(pattern=pattern, path=path)
    if (length(oldNm) < 1) {
        stop("No file found.")
    }
...
collect_holothuria <- function(path="data_output/split_files") {
    holFiles <- list.files(path)
    ...
    for (i in 1:length(holFiles)) {
        ...
        ## write a command that selects the following commands from tmpFile
        ## dwc.family
        ## dwc.genus
        ## dwc.specificEpithet
        ## idigbio.uuid
