...
### I want to use a function that does X, there must be a function or it but I
...
Try to use the correct vocabulary. A package is not the same thing as a
library. Most people will understand what you meant, but others have really
strong feelings about the difference in meaning, but the key point is that it
can make things confusing for people trying to help you. Try to be as precise as
possible in the words you choose.

If possible, try to reduce what doesn't work to the simplest case. If you can
reproduce the problem using a very small `data.frame` instead of your 50,000
rows and 10,000 columns one, include the small one instead. When appropriate,
try to generalize what you are doing so even people who are not in your field
can understand the question.

To share an object with someone else, if it's relatively small, you can use the
function `dput()`, it will output R code that will recreate the exact same
...
* The [R-help](https://stat.ethz.ch/mailman/listinfo/r-help): it is read by a
  ...
  but it can be pretty dry and not very welcoming to new users. If your question
  is valid, you are likely to get an answer very fast but don't expect that it
  will come with smiley faces. Also here more than everywhere else, be sure to
  use correct vocabulary (otherwise you might get an answer pointing to the
  misuse of your words rather than answering your question). You will also have
  more success if your question is about R itself rather than a specific
  package.
* If you question is about a specific package, see if there is a mailing list
